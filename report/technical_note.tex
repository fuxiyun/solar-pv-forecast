...
Two complementary methods assess feature relevance:
\textbf{native split count} (how often LightGBM selects a feature for a tree split) and
\textbf{permutation importance} (how much validation MAE degrades when a feature is shuffled, averaged over 10 repeats).
Permutation importance is generally more trustworthy because it measures predictive contribution rather than structural usage.

\begin{table}[H]
\centering
\caption{Top 10 features: native (split) vs.\ permutation importance.}\label{tab:importance}
\small
\begin{tabular}{@{}clrclr@{}}
\toprule
\multicolumn{3}{c}{\textbf{Native (split count)}} & \multicolumn{3}{c}{\textbf{Permutation (MAE increase)}} \\
\cmidrule(r){1-3} \cmidrule(l){4-6}
Rank & Feature & Splits & Rank & Feature & $\Delta$MAE \\
\midrule
1 & \texttt{proxy\_solar\_mw}   & --- & 1 & \texttt{proxy\_solar\_mw}   & --- \\
2 & \texttt{hour\_sin}          & --- & 2 & \texttt{target\_clearsky\_ghi}     & --- \\
3 & \texttt{hour\_cos}          & --- & 3 & \texttt{actual\_lag\_1d}    & --- \\
4 & \texttt{actual\_lag\_1d}    & --- & 4 & \texttt{target\_solar\_zenith}     & --- \\
5 & \texttt{target\_clearsky\_ghi}     & --- & 5 & \texttt{clearsky\_index}   & --- \\
6 & \texttt{doy\_sin}           & --- & 6 & \texttt{target\_nwp\_ghi\_wm2\_national} & --- \\
7 & \texttt{clearsky\_index}    & --- & 7 & \texttt{ghi\_wm2\_national} & --- \\
8 & \texttt{target\_solar\_zenith}     & --- & 8 & \texttt{target\_hour\_sin}  & --- \\
9 & \texttt{ghi\_wm2\_national} & --- & 9 & \texttt{actual\_lag\_7d}    & --- \\
10 & \texttt{doy\_cos}          & --- & 10 & \texttt{target\_doy\_sin}  & --- \\
\bottomrule
\end{tabular}

\medskip
\footnotesize
Values in the Splits and $\Delta$MAE columns should be filled from\\
\texttt{output/feature\_importance\_native.json} and \texttt{output/feature\_importance\_permutation.json} after running the pipeline.
\end{table}

\textbf{Interpretation.}
The synthetic proxy dominates both rankings, confirming that capacity-weighted irradiance is the primary predictive driver.
Calendar features (\texttt{hour\_sin}/\texttt{hour\_cos}) rank high in native importance (many splits) but lower in permutation importance---each split contributes a small effect, so shuffling them has less impact.
Permutation importance elevates physics-informed features (\texttt{target\_clearsky\_ghi}, \texttt{target\_solar\_zenith}, NWP GHI) relative to their native ranking, indicating the model performs \emph{physics-informed prediction} rather than pure ``smart persistence.''
The 24-hour lag (\texttt{actual\_lag\_1d}) appears in both top~5, providing a persistence anchor; the 7-day lag contributes modestly.

Features whose permutation importance 95\% confidence interval includes zero are candidates for removal but are retained here: the feature set was designed on domain knowledge, and importance analysis validates that design rather than replacing it.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../output/feature_importance_comparison.png}
\caption{Native (split-based) vs.\ permutation importance (validation set).}\label{fig:importance}
\end{figure}
